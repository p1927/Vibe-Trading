...
- **2026-08-04** 🔭 **حيازات المؤسسات، وتفكيك محافظ ETF، وأسواق التنبؤ، وتدفقات عمل البحث**: أُضيفت أربع أدوات للقراءة فقط تعتمد كلها على مصادر عامة مجانية. تقرأ `get_institutional_holdings` إفصاحات SEC 13F-HR — حيازات مدير بعينه، ومن يملك سهماً بعينه، وأكبر المديرين حجماً — مع فروق المراكز ربعاً بربع (جديدة/مزيدة/مخفّضة/مُصفّاة) لخدمة أبحاث العوامل؛ وتحمل إجماليات صفحة الغلاف وحدتها صراحةً، لأن الإفصاحات السابقة لتغيير قواعد 2023 تُعلن بالآلاف، وقراءتها بالدولار تقلّل تقدير المحفظة ألف ضعف وتُفسد ترتيب أي تصنيف بالحجم دون أن يظهر ذلك. أما `etf_holdings` فتفكّك الصندوق إلى مكوّناته عبر واجهة واحدة تشمل أكثر من سوق: يعطي SEC N-PORT الحيازات الأميركية كاملة، أما الصناديق الصينية فلا تُفصح ربعياً إلا عن أكبر عشرة مراكز، غير أن التقريرين النصف‑سنوي والسنوي يحملان الحيازة كاملة وهما ما تقرؤه الأداة: يعود صندوق يتتبّع مؤشر CSI‑300 بـ 342 مركزاً تغطي 98.7% من صافي الأصول بدل عشرة تغطي 22.7%. ويبيّن الرد أيّهما أعطاك ويختمه بفترة التقرير، لأن الحيازة الكاملة هي بحكم طبيعتها الإفصاح الأقدم. وتُدخل `prediction_market` بيانات عقود الأحداث كمُدخل للأبحاث المدفوعة بالأحداث، محوِّلةً أسعار العقود إلى احتمال ضمني موسوم بوحدته حتى لا يُقرأ 0.63 على أنه سعر بالدولار. وتبحث `research_papers` في arXiv وOpenAlex وتستخرج ما يلزم لتنفيذ عامل، تاركةً كل ما لا يَرِد نصّاً في المصدر موسوماً بـ `not stated in source` بدل استنتاجه، وواسمةً أداء الورقة المُعلَن بأنه ادّعاء الورقة لا نتيجة اختبار رجعي لدينا. وإلى جانب ذلك: صار لمحرّك الجدولة أخيراً ما يُشغّله — خمسة قوالب (موجز ما قبل الافتتاح، وتتبّع موسم الأرباح، وفحص المحفظة، وتدفّق أموال الأسهم الصينية، وفروق حيازات المؤسسات) تصف احتياجاتها من البيانات بلغة طبيعية بدل تسمية الأدوات، فتتّسع التغطية دون تعديل القوالب؛ وستة أوامر بحث مؤسسي `/comps` و`/dcf` و`/attrib` و`/memo` و`/earnings` و`/screen`، لكلٍّ منها هيكل تنفيذ ومثال محسوب تتوازن أرقامه؛ وعدسات المستثمرين استُخرجت من إعدادات السرب لتصبح skill مستقلة تتيح إعادة قراءة الأدلة نفسها بإطار مختار. ويكتسب قلب الوكيل مبادئ إخراج لا تستطيع جلسة واحدة تجاوزها — كل رقم يعود إلى الأداة التي أنتجته، والبيانات تحمل زمن سريانها، وما لم تُعِده أداةٌ يُبلَّغ عنه ناقصاً لا يُستحضَر من الذاكرة — إضافةً إلى تحميل تدريجي للـ skill، فما تجاوز حدّ 10k لنتيجة الأداة يعيد بنيته أولاً ويتوسّع عند الطلب بدل أن يصل مبتوراً.
