...
- [x] Full suite (`pytest agent/tests --ignore=agent/tests/e2e_backtest --ignore=agent/tests/test_e2e_harness_v2.py --continue-on-collection-errors -q`) — 4,930 passed, 22 failed, 11 skipped, 21 errors in 505s. All failures/errors are in modules this PR never touches (`test_qveris_tool.py`, `test_run_card.py`, `test_spa_deep_link.py`, `test_strategy_store.py` — Windows-only/FastAPI-204-body issues, the same categories #470's test plan documented as pre-existing on clean `upstream/main`; plus `factors/test_registry.py` OSError). Not independently re-confirmed against a byte-for-byte clean `upstream/main` checkout in this pass — reasoning is module isolation (none of the failing modules import `ccxt_loader`), not a diffed baseline run; re-run on clean `upstream/main` first if you want that stronger confirmation before opening the PR.
