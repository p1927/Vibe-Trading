...
- **2026-05-14** 🧠 **持久记忆 CLI**：新增 `vibe-trading memory list/show/search/forget`，用户无需手动打开 `~/.vibe-trading/memory/` 就能查看、搜索和清理持久记忆。实现通过 lazy imports 保持非 memory CLI 命令启动速度，并在扫描 memory frontmatter 时把解析出的元数据值重新转成字符串，避免 Rich 渲染或 recall scoring 在边界输入上崩溃（[#102](https://github.com/HKUDS/Vibe-Trading/pull/102)，感谢 @Teerapat-Vatpitak）。
- **2026-05-13** 🧭 **Swarm 真实价格 grounding + worker 事件清理**：Swarm run 现在会在分析员启动前预抓取市场数据，并把真实价格注入 worker prompts；标的查询统一走 `_detect_market(code)`，grounding payload 在线程间传递也更安全（[#93](https://github.com/HKUDS/Vibe-Trading/pull/93)）。Worker reports 会持久化到允许的 swarm-run 路径，`report.md` 摘要抽取更稳，preset timeout 已整体拉长，事件预览也会做 redact/sanitize，让流式更新更干净（[#84](https://github.com/HKUDS/Vibe-Trading/pull/84)）。
- **2026-05-12** 🧾 **Trust Layer run cards**：回测现在会在 artifacts 旁边生成可复现的 `run_card.json` 与 `run_card.md`，记录 config 与策略代码 hash、数据来源、标量指标、validation 结果、warnings 和 artifact 校验和。Backtest 工具也会返回 run-card 路径，便于 CLI/API 直接展示证据包。

<details>
<summary>更早的新闻</summary>

- **2026-05-11** 🧭 **Memory slug、swarm 统计与 CLI 预检**：持久记忆生成文件 slug 时现在会保留 CJK 字符，避免中文/日文/韩文笔记发生静默文件名碰撞（[#95](https://github.com/HKUDS/Vibe-Trading/pull/95)，感谢 @voidborne-d）。Swarm run 总量会优先使用 provider 返回的 token usage，并保留原有估算回退（[#94](https://github.com/HKUDS/Vibe-Trading/pull/94)，感谢 @Teerapat-Vatpitak）；CLI run UI 也新增启动预检，用于提前暴露常见环境问题（[#96](https://github.com/HKUDS/Vibe-Trading/pull/96)，感谢 @ykykj）。
...
| **Trust Layer** | 可复现实验卡片：工具轨迹、数据来源、假设与引用 | In Progress |
