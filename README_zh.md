...
- **2026-08-04** 🔭 **机构持仓、ETF 穿透、预测市场与研究工作流**：四个只读数据工具落地，全部基于免费公开数据源。`get_institutional_holdings` 读取 SEC 13F-HR 申报——某管理人的持仓、某只股票被谁持有、规模最大的管理人——并给出季度环比的建仓/加仓/减仓/清仓变化供因子研究使用；封面页总额带显式单位标注，因为 2023 规则变更前的申报以千美元计价，按美元读会把组合低估 1000 倍，并静默打乱任何按规模的排序。`etf_holdings` 用一个接口跨市场穿透到成分持仓：SEC N-PORT 提供美股基金的完整持仓，A 股基金的季报只披露前十大重仓，但半年报与年报披露全部持仓，本工具读取的正是后者：一只沪深 300 ETF 会返回 342 只成分、覆盖净值的 98.7%，而不是十只、覆盖 22.7%。响应会标明本次给出的是哪一种，并附上报告期——因为完整持仓必然来自更早的披露。`prediction_market` 把事件合约数据引入为事件驱动研究的输入，将合约价格转换为带单位标注的隐含概率，使 0.63 不会被误读为美元价格。`research_papers` 检索 arXiv 与 OpenAlex 并提取因子实现所需的要素，源文中没有明确出现的内容一律标记为 `not stated in source` 而非推断补全，论文自身声称的业绩会标注为论文的主张，绝不呈现为我们的回测结果。同时：定时研究调度终于有了可运行的内容——五个模板（盘前简报、财报季追踪、组合体检、A 股资金流向、机构持仓变化），它们以自然语言描述数据需求而不写死工具名，因此数据覆盖增长时无需改动模板；六个机构研究命令 `/comps` `/dcf` `/attrib` `/memo` `/earnings` `/screen`，每个都带执行步骤骨架和算术自洽的完整算例；投资人视角从 swarm preset 中解耦为独立 skill，使同一份证据能用选定的框架重新审视。Agent 核心新增单次会话不可覆盖的输出原则——每个数字都要能追溯到产生它的工具、数据附带信息截止时间、工具没有返回的内容报告为缺失而不是凭记忆补全——以及渐进式 skill 加载：超过 10k 工具返回上限的 skill 现在先返回结构、按需展开，而不是被截断后送达。
