...
- **2026-05-14** 🧠 **永続メモリ CLI**: `vibe-trading memory list/show/search/forget` を追加し、`~/.vibe-trading/memory/` を手で開かなくても永続メモリを確認・検索・削除できるようにしました。non-memory CLI の起動速度を落とさないよう lazy imports を維持し、memory frontmatter の走査では解析されたメタデータ値を文字列へ戻すことで、Rich rendering や recall scoring の境界ケースを堅牢化しました（[#102](https://github.com/HKUDS/Vibe-Trading/pull/102)、@Teerapat-Vatpitak に感謝）。
- **2026-05-13** 🧭 **Real-price swarm grounding + worker event hygiene**: Swarm run は analyst 起動前に市場データを pre-fetch し、実価格で worker prompt を grounding します。銘柄 lookup は `_detect_market(code)` 経由に統一され、grounding payload の thread-safe な受け渡しも強化されました（[#93](https://github.com/HKUDS/Vibe-Trading/pull/93)）。Worker report は許可済み swarm-run パスに永続化され、`report.md` summary extraction、長めの preset timeout、redacted/sanitized event previews により、streaming updates がよりクリーンになりました（[#84](https://github.com/HKUDS/Vibe-Trading/pull/84)）。
- **2026-05-12** 🧾 **Trust Layer run cards**: バックテストは artifacts と並んで再現可能な `run_card.json` と `run_card.md` を出力するようになりました。config と戦略コードの hash、データソース、scalar metrics、validation payload、warnings、artifact checksum を記録します。Backtest ツールも run-card のパスを返すため、CLI/API から evidence package を直接表示できます。

<details>
<summary>過去のニュース</summary>

- **2026-05-11** 🧭 **Memory slug、swarm集計、CLIプリフライト**: 永続メモリのファイル slug 生成で CJK 文字を保持するようになり、中国語/日本語/韓国語ノートの静かなファイル名衝突を防ぎます（[#95](https://github.com/HKUDS/Vibe-Trading/pull/95)、@voidborne-d に感謝）。Swarm run の合計は provider が返す token usage を優先し、従来の推定フォールバックも維持します（[#94](https://github.com/HKUDS/Vibe-Trading/pull/94)、@Teerapat-Vatpitak に感謝）。CLI run UI には一般的な環境問題を早めに見つける起動時プリフライトチェックも入りました（[#96](https://github.com/HKUDS/Vibe-Trading/pull/96)、@ykykj に感謝）。
...
| **Trust Layer** | 再現可能な run card: ツール履歴、データソース、仮定、引用 | In Progress |
