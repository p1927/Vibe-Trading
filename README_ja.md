...
- **2026-08-04** 🔭 **機関投資家保有、ETF ルックスルー、予測市場、リサーチワークフロー**：無料の公開データのみを使う 4 つの読み取り専用ツールを追加しました。`get_institutional_holdings` は SEC 13F-HR を読み取り、運用者の保有銘柄・特定銘柄の保有者・規模上位の運用者を返し、ファクター研究のために四半期ごとの新規／買い増し／削減／全売却の差分も提供します。表紙の合計額には単位を明示します。2023 年の規則変更以前の提出は千ドル単位であり、ドルとして読むとポートフォリオを 1000 倍過小評価し、規模順の並びを静かに狂わせるためです。`etf_holdings` は単一のインターフェースで市場をまたいで構成銘柄まで分解します。米国ファンドは SEC N-PORT により全保有銘柄を取得できます。A 株は四半期報告では上位 10 銘柄のみですが、半期報告と年次報告には全保有が載っており、本ツールはそちらを読みます。CSI300 連動 ETF なら 10 銘柄・純資産の 22.7% ではなく、342 銘柄・98.7% が返ります。どちらを返したかは応答に明示され、報告期も付きます。完全な保有明細は定義上より古い開示だからです。`prediction_market` はイベント契約データをイベントドリブン研究の入力として取り込み、契約価格を単位付きの含意確率に変換するため、0.63 がドル価格と誤読されることはありません。`research_papers` は arXiv と OpenAlex を検索し、ファクター実装に必要な要素を抽出します。原文に明示されていない項目は推測せず `not stated in source` と記し、論文が主張する成績は論文の主張として明示し、当方のバックテスト結果として提示することはありません。あわせて、スケジューリング基盤にようやく実行内容が加わりました。5 つのテンプレート（寄り付き前ブリーフ、決算シーズン追跡、ポートフォリオ点検、A 株資金フロー、機関投資家保有の差分）はツール名を書かず自然言語でデータ要件を記述するため、カバレッジが増えてもテンプレートを編集する必要がありません。さらに機関投資家向けの 6 コマンド `/comps` `/dcf` `/attrib` `/memo` `/earnings` `/screen` は、各手順の骨組みと計算が整合する完全な数値例を備えます。投資家レンズは swarm プリセットから独立した skill として切り出され、同じ証拠を選んだ枠組みで読み直せます。エージェント中核には単一セッションでは上書きできない出力原則を追加しました。すべての数値は生成したツールまで遡れること、データには as-of 時刻を伴うこと、ツールが返さなかったものは記憶で補わず欠落として報告すること。加えて段階的な skill 読み込みにより、10k のツール結果上限を超える skill はまず構造を返し、必要に応じて展開されます。
