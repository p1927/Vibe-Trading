...
_YAHOO_SUMMARY = {
    "incomeStatementHistory": {
        "incomeStatementHistory": [
            {
                "endDate": {"raw": 1735689600, "fmt": "2024-12-31"},
                "totalRevenue": {"raw": 1000000000.0, "fmt": "1B"},
                "grossProfit": {"raw": 600000000.0},
                "operatingIncome": {"raw": 300000000.0},
                "netIncome": {"raw": 200000000.0},
                "dilutedEPS": {"raw": 1.2},
            },
            {
                "endDate": {"raw": 1704067200, "fmt": "2023-12-31"},
                "totalRevenue": {"raw": 900000000.0},
                "netIncome": {"raw": 150000000.0},
                "dilutedEPS": {"raw": 0.9},
            },
        ]
    },
    "balanceSheetHistory": {
        "balanceSheetHistory": [
            {
                "endDate": {"raw": 1735689600, "fmt": "2024-12-31"},
                "totalAssets": {"raw": 2500000000.0},
                "totalLiabilitiesNetMinorityInterest": {"raw": 1200000000.0},
                "stockholdersEquity": {"raw": 1300000000.0},
            }
        ]
    },
    "cashflowStatementHistory": {
        "cashflowStatementHistory": [
            {
                "endDate": {"raw": 1735689600, "fmt": "2024-12-31"},
                "totalCashFromOperatingActivities": {"raw": 400000000.0},
                "totalCashflowsFromInvestingActivities": {"raw": -150000000.0},
                "totalCashFromFinancingActivities": {"raw": -200000000.0},
                "capitalExpenditures": {"raw": -100000000.0},
            }
        ]
    },
    "financialData": {
        "totalCash": {"raw": 3852000.0},
        "totalDebt": {"raw": 136872992.0},
        "totalRevenue": {"raw": 104681000.0},
        "grossProfits": {"raw": 22590000.0},
        "ebitda": {"raw": 2581000.0},
        "operatingCashflow": {"raw": 1894000.0},
        "freeCashflow": {"raw": 20125250.0},
        "returnOnEquity": {"raw": -0.49056998},
        "returnOnAssets": {"raw": -0.01263},
        "currentRatio": {"raw": 0.324},
        "quickRatio": {"raw": 0.143},
        "debtToEquity": {"raw": 189.062},
    },
    "defaultKeyStatistics": {
        "bookValue": {"raw": 0.9122103},
        "netIncomeToCommon": {"raw": -43781000.0},
        "trailingEps": {"raw": -0.55},
    },
}
...
class TestCanadaViaYahoo:
    """Canada (.TO/.V) routes to Yahoo quoteSummary history modules."""

    def test_ca_income_uses_yahoo_and_parses_periods(self):
        with patch(
            "src.tools.financial_statements_tool.yahoo_client.get_quote_summary",
            return_value=_YAHOO_SUMMARY,
        ) as mock_qs:
            text = FinancialStatementsTool().execute(
                code="SGML.V", statement="income", period="annual"
            )

        mock_qs.assert_called_once_with("SGML.V", ["incomeStatementHistory"])
        payload = json.loads(text)
        assert payload["ok"] is True
        assert payload["market"] == "ca"
        assert payload["source"] == "yahoo"
        periods = payload["data"]["SGML.V"]["periods"]
        assert periods[0]["report_date"] == "2024-12-31"
        assert periods[0]["revenue"] == 1000000000.0
        assert periods[0]["currency"] == "CAD"

    def test_ca_indicators_merge_all_histories(self):
        with patch(
            "src.tools.financial_statements_tool.yahoo_client.get_quote_summary",
            return_value=_YAHOO_SUMMARY,
        ) as mock_qs:
            text = FinancialStatementsTool().execute(
                code="SGML.V", statement="indicators", period="annual"
            )

        modules = mock_qs.call_args.args[1]
        assert set(modules) == {
            "incomeStatementHistory",
            "balanceSheetHistory",
            "cashflowStatementHistory",
        ...
        payload = json.loads(text)
        row = payload["data"]["SGML.V"]["periods"][0]
        assert row["revenue"] == 1000000000.0
        assert row["assets"] == 2500000000.0
        assert row["operating_cash_flow"] == 400000000.0

    def test_ca_balance_falls_back_to_snapshot_when_history_empty(self):
        summary = dict(_YAHOO_SUMMARY)
        ...
        with patch(
            ...
            text = FinancialStatementsTool().execute(
                code="SGML.V", statement="balance", period="annual"
            )

        modules = mock_qs.call_args.args[1]
        assert "balanceSheetHistory" in modules
        assert {"financialData", "defaultKeyStatistics"} <= set(modules)
        payload = json.loads(text)
        assert payload["ok"] is True
        row = payload["data"]["SGML.V"]["periods"][0]
        assert row["report_date"] == "latest"
        assert row["snapshot"] is True
        assert row["cash"] == 3852000.0
        assert row["total_debt"] == 136872992.0

    def test_ca_quote_summary_failure_surfaces_error(self):
        with patch(
            "src.tools.financial_statements_tool.yahoo_client.get_quote_summary",
            side_effect=RuntimeError("yahoo 401"),
        ):
            text = FinancialStatementsTool().execute(code="SGML.V")

        payload = json.loads(text)
        assert payload["ok"] is False
        assert "yahoo" in payload["error"]
