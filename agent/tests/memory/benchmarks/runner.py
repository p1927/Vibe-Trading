...
def compute_importance_weight(record: MemoryRecord) -> float:
    ...
    Then used as: final_score = relevance × (0.5 + 0.5 × importance)
    This ensures importance provides a controlled boost [0.5x, 1.0x] on top of
    relevance, matching the production `find_relevant` behavior.
...
def retrieve_top_k(
    ...
    for record in corpus:
        ...
        if treatment:
            ...
            # Range [0.98, 1.0] — lifecycle signal for near-tie resolution
