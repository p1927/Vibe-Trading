...
async def maybe_resume_auto_paper_session(store, session_service) -> dict[str, Any] | None:
    """If paper session is active, bootstrap a fresh Vibe turn with resume prompt."""
    try:
        ...
        logger.debug("auto paper resume skipped — trade stack unavailable", exc_info=True)
        return None

    session = load_session()
    if not session.get("enabled") or not is_agent_session_active():
        return None
    ...
    session["needs_fresh_vibe_session"] = True
