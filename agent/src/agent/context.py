...
from src.agent.session_prompts import (
    NEWS_SCENARIO_SYSTEM_PROMPT as _NEWS_SCENARIO_SYSTEM_PROMPT,
    OBSERVE_AUTONOMOUS_SYSTEM_PROMPT as _OBSERVE_AUTONOMOUS_SYSTEM_PROMPT,
    ORCHESTRATOR_SYSTEM_PROMPT as _ORCHESTRATOR_SYSTEM_PROMPT,
    news_scenario_skill_block,
    orchestrator_skill_block,
)
...
class ContextBuilder:
    ...
    def build_system_prompt(self, user_message: str = "") -> str:
        ...
        if session_kind == "autonomous_orchestrator":
            ...
            skill_block = orchestrator_skill_block(self.skills_loader)
            if skill_block:
                base = f"{base}\n{skill_block}"
            try:
    ...
    def _is_stale_outage_memory(entry: "MemoryEntry") -> bool:
        ...
        from src.memory.outage_staleness import is_stale_outage_entry

        return is_stale_outage_entry(entry.title, entry.description, entry.created_at)
        ...
        if self._persistent_memory:
            try:
                ...
                try:
                    ...
                    pass
                if recalls:
                    from src.memory.outage_staleness import UNVERIFIED_RECALL_SUFFIX

                    lines = []
                    for r in recalls:
                        line = f"- **{r.title}** ({r.memory_type}): {r.body[:500]}"
                        if self._is_stale_outage_memory(r):
                            line += UNVERIFIED_RECALL_SUFFIX
                        lines.append(line)
                    recall_block = "\n".join(lines)
                    enriched = (
                        f"<recalled-memories>\n{recall_block}\n</recalled-memories>\n\n"
