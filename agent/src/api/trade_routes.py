...
_WIDGET_ID_RE = re.compile(r"tp_[A-Z0-9_]+_[0-9a-f]{12}")
...
def _widget_id_from_preview(preview: str) -> Optional[str]:
    match = re.search(r'"widget_id"\s*:\s*"(tp_[^"]+)"', preview or "")
...
def _openalgo_basket_url() -> tuple[str, str]:
...
def execute_basket(
    ...
    host, api_key = _openalgo_basket_url()
    ...
    return ExecuteBasketResponse(
        ...
        message=str(body_json.get("message") or "Basket submitted"),
    )
