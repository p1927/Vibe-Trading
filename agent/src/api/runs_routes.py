...
def _list_runs_sync(runs_dir: Path, limit: int, run_info_cls: Any) -> List[Any]:
    """Blocking body of ``list_runs`` — file-existence checks and small JSON/
    text/CSV reads across up to ``limit`` run directories. Must run off the
    event loop (see ``run_in_threadpool`` at the call site)."""
...
def register_runs_routes(
    ...
    async def list_runs(limit: int = 20):
        ...
        return await run_in_threadpool(_list_runs_sync, runs_dir, limit, RunInfo)
