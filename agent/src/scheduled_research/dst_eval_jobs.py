...
async def dispatch_dst_eval_job(job: ScheduledResearchJob) -> None:
    ...
    await run_logged(job, in_child(dispatch_dst_eval_job_sync))
...
def register_default_dst_eval_jobs(store: ScheduledResearchJobStore) -> int:
    ...
    enabled = is_dst_eval_scheduler_enabled()
    reason = None if enabled else "DST_EVAL_ENABLE_SCHEDULER disabled (D80)"
    ...
    for job_id, prompt, cron, job_type in jobs:
        ...
        if store.get(job_id) is None:
            ...
            logger.info("registered dst_eval job %s (%s, paused=%s)", job_id, cron, not enabled)
            created += 1
