...
- **2026-08-04** 🔭 **기관 보유, ETF 룩스루, 예측 시장, 리서치 워크플로**: 무료 공개 데이터만 사용하는 읽기 전용 도구 4개가 추가되었습니다. `get_institutional_holdings`는 SEC 13F-HR 공시를 읽어 특정 운용사의 보유 종목, 특정 종목의 보유자, 규모 상위 운용사를 제공하며 팩터 연구를 위해 분기 대비 신규/증가/축소/청산 변화도 함께 제공합니다. 표지 합계에는 단위를 명시합니다. 2023년 규정 변경 이전 공시는 천 달러 단위여서 달러로 읽으면 포트폴리오를 1000배 과소평가하고 규모 기준 정렬을 조용히 어긋나게 만들기 때문입니다. `etf_holdings`는 하나의 인터페이스로 시장을 넘나들며 구성 종목까지 분해합니다. 미국 펀드는 SEC N-PORT로 전체 보유를 얻습니다. A주는 분기 보고서에서 상위 10개만 공시하지만 반기·연차 보고서에는 전체 보유가 실리며 이 도구는 그것을 읽습니다. CSI300 추종 ETF라면 10종목·순자산 22.7%가 아니라 342종목·98.7%가 돌아옵니다. 어느 쪽을 반환했는지는 응답에 표시되고 보고 기준일도 함께 붙습니다. 완전한 보유 명세는 본질적으로 더 오래된 공시이기 때문입니다. `prediction_market`은 이벤트 계약 데이터를 이벤트 드리븐 연구의 입력으로 가져오며, 계약 가격을 단위가 표시된 내재 확률로 변환해 0.63이 달러 가격으로 오독되지 않게 합니다. `research_papers`는 arXiv와 OpenAlex를 검색해 팩터 구현에 필요한 요소를 추출합니다. 원문에 명시되지 않은 항목은 추론하지 않고 `not stated in source`로 표시하며, 논문이 주장하는 성과는 논문의 주장으로 표기할 뿐 우리의 백테스트 결과로 제시하지 않습니다. 아울러 스케줄링 런타임에 드디어 실행할 내용이 생겼습니다. 다섯 개 템플릿(장 시작 전 브리프, 실적 시즌 추적, 포트폴리오 점검, A주 자금 흐름, 기관 보유 변화)은 도구 이름을 적지 않고 자연어로 데이터 요구를 기술하므로 커버리지가 늘어도 템플릿을 고칠 필요가 없습니다. 기관 리서치 명령 6개 `/comps` `/dcf` `/attrib` `/memo` `/earnings` `/screen`는 각각 실행 골격과 산술이 맞아떨어지는 완전한 예제를 담고 있습니다. 투자자 렌즈는 swarm 프리셋에서 분리되어 독립 skill이 되었고, 같은 증거를 선택한 프레임으로 다시 읽을 수 있습니다. 에이전트 코어에는 단일 세션이 뒤집을 수 없는 출력 원칙이 추가되었습니다. 모든 숫자는 그것을 만든 도구까지 추적되어야 하고, 데이터는 as-of 시각을 동반하며, 도구가 반환하지 않은 것은 기억으로 채우지 않고 누락으로 보고합니다. 여기에 점진적 skill 로딩이 더해져 10k 도구 결과 상한을 넘는 skill은 먼저 구조를 반환하고 요청에 따라 펼쳐집니다.
