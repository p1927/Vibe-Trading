...
- **2026-05-14** 🧠 **영구 메모리 CLI**: `vibe-trading memory list/show/search/forget`을 추가해 `~/.vibe-trading/memory/`를 직접 열지 않고도 영구 메모리를 확인, 검색, 정리할 수 있습니다. non-memory CLI 시작 속도를 유지하기 위해 lazy import를 사용하고, memory frontmatter 스캔 시 파싱된 메타데이터 값을 다시 문자열로 강제 변환해 Rich 렌더링이나 recall scoring의 경계 입력을 더 안전하게 처리합니다([#102](https://github.com/HKUDS/Vibe-Trading/pull/102), @Teerapat-Vatpitak 감사합니다).
- **2026-05-13** 🧭 **실가격 swarm grounding + worker event hygiene**: Swarm run이 analyst 시작 전에 시장 데이터를 pre-fetch하고 실제 가격으로 worker prompt를 grounding합니다. 심볼 조회는 `_detect_market(code)` 경로로 통일되고 grounding payload의 thread-safe 전달도 강화되었습니다([#93](https://github.com/HKUDS/Vibe-Trading/pull/93)). Worker report는 허용된 swarm-run 경로에 영구 저장되며, `report.md` summary extraction, 더 긴 preset timeout, redacted/sanitized event preview로 streaming update가 더 깔끔해졌습니다([#84](https://github.com/HKUDS/Vibe-Trading/pull/84)).
