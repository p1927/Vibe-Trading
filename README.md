...
- **2026-08-04** 🔭 **Institutional holdings, ETF look-through, prediction markets, and research workflows**: Four read-only data tools land, all on free public sources. `get_institutional_holdings` reads SEC 13F-HR filings — a manager's book, who holds a given ticker, the largest managers — with quarter-over-quarter position diffs for factor work; cover-page totals carry an explicit unit, because filings before the 2023 rule change report in thousands and reading one as dollars understates a portfolio 1000× and silently mis-orders any ranking by size. `etf_holdings` resolves a fund to its constituents through one interface across markets: SEC N-PORT gives the full US book, while A-share funds publish only their top ten holdings quarterly, but the semi-annual and annual reports carry the whole book, and those are what this reads: a CSI-300 tracker comes back with 342 positions covering 98.7% of net assets rather than ten covering 22.7%. The response says which one it gave you and stamps the report period, since the complete book is by definition the older disclosure. `prediction_market` brings event-contract data in as an event-driven input, converting contract prices to labelled implied probability so 0.63 is never read as a dollar price. `research_papers` searches arXiv and OpenAlex and extracts what a factor implementation needs, leaving anything not literally present in the source marked `not stated in source` rather than inferring it — and labelling a paper's reported performance as the paper's claim, never as our backtest. Alongside them: the scheduling runtime finally ships something to run — five templates (premarket brief, earnings-season tracker, portfolio checkup, A-share money flow, institutional-holdings diff) that state their data needs in natural language rather than naming tools, so coverage can grow without editing them; six institutional research commands `/comps` `/dcf` `/attrib` `/memo` `/earnings` `/screen`, each with an execution skeleton and a worked example whose arithmetic ties out; and investor lenses lifted out of swarm presets into a standalone skill so the same evidence can be re-read through a chosen framework. The agent core gains output principles that a single session cannot override — every number traces to the tool that produced it, data carries its as-of time, and anything a tool did not return is reported missing rather than recalled — plus progressive skill loading, so a skill longer than the 10k tool-result cap returns its structure and expands on request instead of arriving truncated.
